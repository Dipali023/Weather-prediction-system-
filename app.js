...
//   Nagpur, India coordinates: lat=21.15, lon=79.09
// ═══════════════════════════════════════════════════════
async function loadForecast() {
  ...
  try {
    const url = 'https://api.open-meteo.com/v1/forecast?latitude=21.15&longitude=79.09&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weathercode,windspeed_10m_max&current_weather=true&timezone=Asia%2FKolkata&forecast_days=7';
    ...
    setEl('locationText', 'Nagpur, Maharashtra — Live API Data');
    const apiPill = document.getElementById('apiPill');
    if (apiPill) { apiPill.textContent = '🌐 Open-Meteo: Connected'; apiPill.className = 'api-pill connected'; }
    if (forecastSrc) { forecastSrc.textContent = '✅ Open-Meteo API — Live 7-Day Forecast'; forecastSrc.className='forecast-source-pill api-ok'; }
    logML('info', 'Open-Meteo API connected — real forecast data loaded successfully');
  } catch (e) {
    useFallbackForecast();
...
async function fetchSunTimes() {
  try {
    const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=21.15&longitude=79.09&daily=sunrise,sunset&timezone=Asia%2FKolkata&forecast_days=1');
    const json = await res.json();
